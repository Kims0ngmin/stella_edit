...
void orb_extractor::extract(const cv::_InputArray& in_image, const cv::_InputArray& in_image_mask,
    ...
    if (num_keypts == 0) {
        ...
        // ?!?!!?
        descriptors_ori = out_descriptors.getMat();
    }
    ...
    for (unsigned int level = 0; level < orb_params_->num_levels_; ++level) {
        ...
        // for (unsigned int level = 0; level < orb_params_->num_levels_; ++level) {
        //     const cv::Mat& current_image = image_pyramid_.at(level);
        //     int height = current_image.rows;
        //     int width = current_image.cols;

        //     spdlog::info("Level {}: Resolution - {} x {}", level, width, height);
        //     for (const auto& keypoint : keypts_at_level) {
        //         spdlog::info("Origin :: Level: {}, KeyPoint: x={}, y={}, size={}, angle={}, response={}, octave={}, class_id={}",
        //             level, keypoint.pt.x, keypoint.pt.y, keypoint.size,
        ...
        cv::Mat descriptors_at_level = descriptors_ori.rowRange(offset, offset + num_keypts_at_level);
        ...
        // // Logging keypts_at_level information
        // for (const auto& keypoint : keypts_at_level) {
        //     spdlog::info("After :: Level: {}, KeyPoint: x={}, y={}, size={}, angle={}, response={}, octave={}, class_id={}",
        //             level, keypoint.pt.x, keypoint.pt.y, keypoint.size,
        //             keypoint.angle, keypoint.response, keypoint.octave, keypoint.class_id);
...
void orb_extractor::create_rectangle_mask(const unsigned int cols, const unsigned int rows) {
    ...
    spdlog::warn("Invalid create_rectangle_mask");
}
...
void orb_extractor::correct_keypoint_scale(std::vector<cv::KeyPoint>& keypts_at_level, const unsigned int level) const {
    ...
    // spdlog::info("scale_factors_.at(level) : {}", scale_at_level);
...
void orb_extractor::compute_orb_descriptors(const cv::Mat& image, const std::vector<cv::KeyPoint>& keypts, cv::Mat& descriptors) const {
    ...
    // std::cout<<"ori= "<< descriptors << std::endl;
}
